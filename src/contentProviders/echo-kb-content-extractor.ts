...
export class EchoKnowledgeBaseExtractor extends BaseContentExtractor {
  ...
  async detect(page: Page): Promise<boolean> {
    return await page.evaluate(() => {
      ...
      return !!(
        ...
        (
          document.body.classList.contains('single-epkb_post_type_1') || 
          document.body.innerHTML.includes('epkb') ||
          document.body.innerHTML.includes('eckb')
        )
      );
    });
  }
  ...
  async extract(page: Page): Promise<PageContent> {
    return await page.evaluate(() => {
      // Get the page title
      const pageTitle = document.querySelector('.eckb-article-title, .epkb-article-header__title')?.textContent?.trim() || 
